...
Аналітика українського IT-ринку праці: ETL (work.ua + DOU + robota.ua →
LLM-каскад → PostgreSQL) + FastAPI + React-дашборд. Окремий Docker-стек
у мережі `homelab`, проксі через Caddy.

> Канонічна інструкція деплою — `docs/DEPLOY.md` у репозиторії.
> Тут — лише специфіка homelab (шляхи `/data/...`, мережа `homelab`, Caddy).
> Зміни процедури деплою вноси спочатку в `docs/DEPLOY.md`.
...
cd 503work
git checkout v2.0.0                  # деплоїмо тег, не гілку
...
CEREBRAS_API_KEY / GROQ_API_KEY      # LLM-каскад — хоча б один (рекомендовано обидва)
# GEMINI_API_KEY / MISTRAL_API_KEY   # опційно — додають денної квоти
JWT_SECRET                           # ОБОВ'ЯЗКОВО: openssl rand -hex 32
...
NLP іде через LLM-каскад у **drain-режимі**: один прогін крутить порції, поки черга
не спорожніє або денні бюджети всіх провайдерів не вичерпаються. Тобто **один прогін/добу
вже вибирає денну стелю** — окремий режим backlog не потрібен.

Денна стеля = сума денних квот увімкнених провайдерів (Cerebras 1М TPD ≈ 1000 записів +
Groq 500K TPD ≈ 590 + Gemini 500 RPD) ≈ **1600–2000 записів/добу**. Хочеш більше — додай
ще провайдерів (`GEMINI_API_KEY` / `MISTRAL_API_KEY`), а не частіший cron.

```cron
# один drain-прогін/добу о 05:00 — каскад сам добирає до денної стелі
